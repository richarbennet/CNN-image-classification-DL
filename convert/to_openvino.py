...
def convert(model, variant, num_classes, checkpoint, size, precision):
    ...
    onnx_path = checkpoint.parent / f"{checkpoint.name.split('.')[0]}_openvino.onnx"
    openvino_path = checkpoint.parent / f'{checkpoint.name.split(".")[0]}_openvino_{precision}'
    ...
    print(f"Finished converting and saved model at `{openvino_path}`")
