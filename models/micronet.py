    ...
    def __init__(self, c1, c2, k=3, s=1, 
        ...
        if gs1[0] == 0:
            self.layers = nn.Sequential(
                ...
                nn.ChannelShuffle(gs1[1]),
                nn.ChannelShuffle(ch2 // 2) if y2 != 0 else nn.Sequential(),
                PWConv(ch2, c2, (g1, g2)),
                DYShiftMax(c2, c2, [1.0, 0.0], [0.0, 0.0], False, (g1, g2), reduction//2) if y3 > 0 else nn.Sequential(),
                nn.ChannelShuffle(g2),
                nn.ChannelShuffle(c2 // 2) if c2%2 == 0 and y3 != 0 else nn.Sequential()
            )
        elif g2 == 0:
            self.layers = nn.Sequential(
                PWConv(c1, ch2, gs1),
            ...
            self.layers = nn.Sequential(
                ...
                nn.ChannelShuffle(ch2 // 4) if y1 != 0 and y2 != 0 else nn.Sequential() if y1 == 0 and y2 == 0 else nn.ChannelShuffle(ch2 // 2),
                PWConv(ch2, c2, (g1, g2)),
                DYShiftMax(c2, c2, [1.0, 0.0], [0.0, 0.0], False, (g1, g2), reduction=reduction//2 if c2 < ch2 else reduction) if y3 > 0 else nn.Sequential(),
                nn.ChannelShuffle(g2),
                nn.ChannelShuffle(c2 // 2) if y3 != 0 else nn.Sequential()
            )
...
class MicroNet(nn.Module):
    def __init__(self, model_name: str = 'M0', pretrained: str = None, num_classes: int = 1000, *args, **kwargs) -> None:
        super().__init__()
        self.inplanes = 64

        assert model_name in micronet_settings.keys(), f"MicroNet model name should be in {list(micronet_settings.keys())}"
        stem_ch, stem_groups, out_ch, init_a, init_b, cfgs = micronet_settings[model_name]

        layers = [Stem(3, stem_ch, 2, stem_groups)]

        input_channel = stem_ch

        for s, c, ks, c1, c2, g1, g2, c3, g3, g4, y1, y2, y3, r in cfgs:
            layers.append(MicroBlock(input_channel, c, ks, s, (c1, c2), (g1, g2), (c3, g3, g4), (y1, y2, y3), r, init_a, init_b))
            input_channel = c
